use bank_core::bank::{implements::memory::storage::{MemAccountStorage, MemTransactionStorage}, storage::AccountStorage, Bank};
...
fn main() {
    ...
    let mut server = Server::new(handler, None, None);
    let _ = server.run();
}
